...
class SpacedSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        ray_bundle_batch_shape = ray_bundle.origins.shape[:-1]
        ...
        if self.train_stratified and self.training:
            if self.single_jitter:
                t_rand = torch.rand(
                    (*ray_bundle_batch_shape, 1), dtype=bins.dtype, device=bins.device
                )
            else:
                t_rand = torch.rand(
                    (*ray_bundle_batch_shape, num_samples + 1),
                    dtype=bins.dtype,
                    device=bins.device,
                )
            ...
            bins = bins.repeat(*ray_bundle_batch_shape, 1)
